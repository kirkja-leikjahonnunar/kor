...
onready var USERNAME_IN := $MainLoginWindow/Username
onready var PASSWORD_IN := $MainLoginWindow/Password
...
func AttemptLogin(ip, port):
	if USERNAME_IN.text == "" or PASSWORD_IN.text == "":
		...
		# TODO: Set timer.
		Gateway.ConnectToGatewayServer(username, password, ip, port)
