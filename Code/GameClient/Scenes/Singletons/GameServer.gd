...
var delta_latency := 0.0
var decimal_collector := 0.0
var latency_array := []

func _physics_process(delta):
	#TODO: this probably needs to be int to preserve millisecond accuracy
...
func connection_succeeded():
	...
	rpc_id(1, "ServerTimeRequest", Time.get_unix_time_from_system()) # note this time is a float
		
	#SceneTreeTimer doesn't do what we want currently: get_tree().create_timer(0.5).timeout.connect(RequestLatency)
	var timer = Timer.new()
...
# This is returned from GameServer
@rpc
